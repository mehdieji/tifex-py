from feature_extraction.statistical_feature_calculators_edited import *
from feature_extraction.spectral_features_calculators import *
from feature_extraction.settings import StatisticalFeatureParams
from scipy.signal import welch


def calculate_statistical_features(signal, signal_name, params=None):
    """
    Calculates all statistical features for a given signal.
    """
    # If no parameters are provided, use the default ones
    if params is None:
    ...
    # Harmonic mean
    feats.extend(calculate_harmonic_mean(signal))
    feats_names.append(f"{signal_name}_harmonic_mean")

    # Trimmed mean
    trimmed_means = calculate_trimmed_mean(signal, params.trimmed_mean_thresholds)
    feats.extend(trimmed_means)
    for threshold in params.trimmed_mean_thresholds:
    ...
    trimmed_mean_abs = calculate_trimmed_mean_abs(signal, params.trimmed_mean_thresholds)
    ...
    # Quantiles
    feats.extend(calculate_quantile(signal, params.q))
    for i in (params.q):
        if i == 0.25:
    ...
    # Entropy
    feats.extend(calculate_entropy(signal, params.window_size))
    feats_names.append(f"{signal_name}_entropy")
    # Permutation entropy
    feats.extend(calculate_permutation_entropy(signal, params.window_size, params.permutation_entropy_order, params.permutation_entropy_delay))
    feats_names.append(f"{signal_name}_permutation_entropy")
    # Tsallis entropy
    feats.extend(calculate_tsallis_entropy(signal, params.window_size, params.tsallis_q_parameter))
    feats_names.append(f"{signal_name}_tsallis_entropy")
    # Renyi entropy
    feats.extend(calculate_renyi_entropy(signal, params.window_size, params.renyi_alpha_parameter))
    ...
    feats.extend(calculate_svd_entropy(signal, params.svd_entropy_order, params.svd_entropy_delay))
    ...
    # Auto-correlation
    feats.extend(calculate_mean_auto_correlation(signal, params.n_lags_auto_correlation))
    feats_names.append(f"{signal_name}_mean_of_auto_corr_lag_1_to_{params.n_lags_auto_correlation}")
    # High-order moments
    feats.extend(calculate_higher_order_moments(signal, params.moment_orders))
    for order in params.moment_orders:
        feats_names.append(f"{signal_name}_moment_order_{order}")
    # Median absolute deviation
    feats.extend(calculate_median_absolute_deviation(signal, params.adjusted))
    ...
    feats.extend(calculate_slope_sign_change(signal, params.ssc_threshold))
    feats_names.append(f"{signal_name}_no._of_slope_sign_changes")
    # Higuchi Fractal Dimensions
    feats.extend(calculate_higuchi_fractal_dimensions(signal, params.higuchi_k_values))
    for k in params.higuchi_k_values:
...
def calculate_frequency_features(signal, signal_name, params=None):
    ...
    length = len(signal)
    freqs_spectrum = np.abs(np.fft.fftfreq(length, 1.0 / params.fs)[:length // 2 + 1])

    # Calculating the power spectral density using Welch's method.
    freqs_psd, psd = welch(signal, fs=params.fs)
    ...
    feats.extend(calculate_spectral_slope_linear(freqs_spectrum, spectrum_magnitudes))
    ...
    feats.extend(calculate_mean_frequency(freqs_spectrum, spectrum_magnitudes))
    ...
    feats.extend(calculate_frequency_winsorized_mean(freqs_spectrum, spectrum_magnitudes))
    ...
    feats.extend(calculate_spectral_strongest_frequency_phase(freqs_spectrum, spectrum))
    ...
    feats.extend(calculate_spectral_spread_shift(freqs_spectrum, spectrum_magnitudes))
    feats_names.append(f"{signal_name}_spectral_spread_shift")

    # Spectral Entropy Shift: The change in spectral entropy over time.
    feats.extend(calculate_spectral_entropy_shift(spectrum_magnitudes))
    ...
    feats.extend(calculate_frequency_variance(freqs_spectrum, spectrum_magnitudes))
    ...
    feats.extend(calculate_frequency_std(freqs_spectrum, spectrum_magnitudes))
    feats_names.append(f"{signal_name}_frequency_std")

    # Frequency Range: Range of the frequencies.
    feats.extend(calculate_frequency_range(freqs_spectrum))
    ...
    feats.extend(calculate_frequency_trimmed_mean(freqs_spectrum, spectrum_magnitudes))
